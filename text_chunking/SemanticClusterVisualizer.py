...
from .splitting.SemanticSplitGenerator import SemanticSplitGenerator
from .utils.SemanticGroupUtils import SemanticGroupUtils
from .llm.chain import ChunkSummarizer
import logging
import numpy as np
...
class SemanticClusterVisualizer:
    ...
    def __init__(
        ...
        base_url_embeddings: str = "https://api.openai.com/v1",
        ...
        """
        self.llm = ChatOpenAI(model=llm_model, temperature=temperature, api_key=api_key, base_url= base_url_llm)
        self.embeddings = OpenAIEmbeddings(model= embeddings_model, api_key=api_key, base_url= base_url_embeddings)
    ...
    def embed_original_document_splits(self, doc_splits: List[str]) -> List[np.ndarray]:
        ...
        Processes documents one by one with a progress bar.
        ...
        """
        original_split_embeddings = []
        
        # Process each document individually with progress tracking
        for doc in tqdm(doc_splits, desc="Generating embeddings"):
            # Embed single document and append to results
            embedding = self.embeddings.embed_documents([doc])[0]
            original_split_embeddings.append(embedding)
    ...
    def embed_semantic_groups(self, semantic_groups: List[str]) -> List[np.ndarray]:
        ...
        # Process each semantic group individually with progress tracking
        for group in tqdm(semantic_groups, desc="Generating semantic group embeddings"):
            # Embed single semantic group and append to results
            embedding = self.embeddings.embed_documents([group])[0]
            semantic_group_embeddings.append(embedding)
    ...
    def vizualize_semantic_groups(
        ...
        if n_clusters > 1:
            cluster_linkage = self.split_visualizer.create_hierarchical_clustering(
                semantic_group_embeddings, plot=False 
            )
